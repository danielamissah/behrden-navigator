...
interface AppState {
  language: Language;
  progress: Record<string, UserProgress>;  // keyed by procedure_id
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  // Creates a progress entry when user starts a procedure
  ...
  // Marks a step as completed and updates procedure status
  completeStep: (procedureId, stepId) => {
    set((state) => {
      ...
      const completedSteps = Array.from(
        new Set([...existing.completed_steps, stepId])
      );
      return {
        progress: {
          ...state.progress,
